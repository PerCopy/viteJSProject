...
TITLE="Instant Event ${CASE_SUFFIX}"
DESCRIPTION="Event with same start and end time ${CASE_SUFFIX}"
LOCATION="Virtual Meeting Room ${CASE_SUFFIX}"
START_DATE="2024-08-01T12:00:00Z"
END_DATE="2024-08-01T12:00:00Z"
TMP_DIR="$(mktemp -d)"
RESPONSE_FILE="$TMP_DIR/create_event_equal_start_and_end_dates.json"
STATUS_FILE="$TMP_DIR/create_event_equal_start_and_end_dates.status"
LIST_FILE="$TMP_DIR/create_event_equal_start_and_end_dates_list.json"
...
if (body.startDate !== body.endDate) process.exit(1);
...
if (event.startDate !== event.endDate) process.exit(1);
...
echo "CODEVALID_TEST_ASSERTION_OK:create_event_equal_start_and_end_dates"
