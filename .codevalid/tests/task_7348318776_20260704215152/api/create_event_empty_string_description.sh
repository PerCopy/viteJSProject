...
CASE_SUFFIX="$(date +%s)-$$"
EVENT_TITLE="Event With Empty Desc ${CASE_SUFFIX}"
EVENT_LOCATION="Venue XYZ ${CASE_SUFFIX}"
START_DATE="2025-10-01T09:00:00Z"
END_DATE="2025-10-01T18:00:00Z"
RESPONSE_FILE="/tmp/create_event_empty_string_description_${CASE_SUFFIX}.json"
STATUS_FILE="/tmp/create_event_empty_string_description_${CASE_SUFFIX}.status"
EVENT_ID=""
cleanup_files() {
...
# Given
: "Prepare a unique event payload with an explicitly empty description string"

# When
curl -sS -o "$RESPONSE_FILE" -w '%{http_code}' \
  -X POST "$BASE_URL/api/events" \
  -H 'Content-Type: application/json' \
  --data "{\"title\":\"${EVENT_TITLE}\",\"description\":\"\",\"location\":\"${EVENT_LOCATION}\",\"startDate\":\"${START_DATE}\",\"endDate\":\"${END_DATE}\"}" > "$STATUS_FILE"
...
echo "CODEVALID_TEST_ASSERTION_OK:create_event_empty_string_description"
